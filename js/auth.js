...
document.getElementById('loginForm').addEventListener('submit',async e=>{
  e.preventDefault(); if(!B13_CONFIGURED) return message('loginMessage','Supabase não configurado no Vercel. Adicione VITE_SUPABASE_URL e VITE_SUPABASE_ANON_KEY e faça um novo deploy.','error');
...
document.getElementById('registerForm').addEventListener('submit',async e=>{
  e.preventDefault(); if(!B13_CONFIGURED) return message('registerMessage','Supabase não configurado no Vercel. Adicione VITE_SUPABASE_URL e VITE_SUPABASE_ANON_KEY e faça um novo deploy.','error');
